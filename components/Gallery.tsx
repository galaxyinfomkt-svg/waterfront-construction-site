    ...
    <>
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        {images.map((g, i) => (
          ...
          >
            <Image src={g} alt={`Project photo ${i + 1}`} fill quality={60} className="object-cover zoomimg" />
      ...
      {active !== null && (
        <div onClick={close} className="fixed inset-0 z-[70] bg-black/85 backdrop-blur-sm grid place-items-center p-4 reveal">
          ...
          <div className="relative w-full max-w-5xl aspect-[4/3]" onClick={(e) => e.stopPropagation()}>
            <Image src={images[active]} alt={`Project photo ${active + 1}`} fill quality={60} className="object-contain rounded-xl" sizes="100vw" />
          </div>
